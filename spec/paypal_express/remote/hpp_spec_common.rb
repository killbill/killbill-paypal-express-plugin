shared_examples 'hpp_spec_common' do |options = {}|
  ...
  it 'should return an empty list of plugin info if payment does not exist' do
    payment_plugin_info = @plugin.get_payment_info(@pm.kb_account_id, SecureRandom.uuid, merge_properties([], options), @call_context)
  ...
  it 'should generate forms correctly' do
    ...
    n = 2
    1.upto(n) do
      form = @plugin.build_form_descriptor(@pm.kb_account_id, @form_fields, merge_properties([], options), @call_context)
      validate_form(form)
  ...
  it 'should fix the unknown transactions to success' do
    [:AUTHORIZE, :CAPTURE, :REFUND].each do |type|
      ...
      # Sleep 5s to allow details_for endpoint be populated with data from PayPal.
      sleep(15)
